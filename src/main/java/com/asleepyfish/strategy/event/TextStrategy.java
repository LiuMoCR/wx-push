...
public class TextStrategy implements WxEventStrategy {
    ...
    private void createImage(String acceptContent, String openId) throws Exception {
        ...
        ImageResult results = new ImageResult();
        List<String> errorResult = Lists.newArrayList();
        // 初始化标记status = 0，表示解答成功
        int status = 0;
        try {
        ...
        for (Image result : results.getData()) {
            AiQa aiQa = new AiQa();
            aiQa.setUser(openId);
            aiQa.setQuestion(acceptContent);
            aiQa.setAnswer(result.getB64Json());
            aiQa.setStatus(status);
            aiQaRepository.save(aiQa);
            if (status == -1) {
                wxMpKefuMessage.setMsgType(WxMessageType.TEXT.getType());
            ...
            WxMediaUploadResult wxMediaUploadResult = getMediaUploadResult(result.getB64Json());
